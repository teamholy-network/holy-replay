...
import de.teamholy.replay.utils.MathUtils;
...
public class PacketNPCOld implements INPC{
	...
	public void updateNameTag(String tabListPrefix, String tabListSuffix,
		...
		if (tabListPrefix != null || tabListSuffix != null) {
			...
			teamPacket.setPlayers(Arrays.asList(new String[] { this.name }));
			...
			for(Player player : Arrays.asList(this.visible)) {
				if(player != null) {
					...
					createTeam.setPlayers(Arrays.asList(new String[] { this.name }));
